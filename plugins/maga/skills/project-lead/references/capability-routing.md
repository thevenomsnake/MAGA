...
| Work responsibility | Profile key |
| --- | --- |
| Product conversation, decisions, and integration | `project-lead` |
| Source-backed external facts | `research` |
| Interaction and visual exploration | `prototype` |
| Bounded product implementation | `delivery` |
| Evidence-led failure investigation | `diagnosis` |
| Independent acceptance and quality review | `review` |
| Privacy, migration, permissions, and release | `release` |

The Product Owner may explicitly configure any responsibility in the settings panel.
Unconfigured fields inherit the host default, including missing responsibilities
in a partially saved configuration. Resolve a saved profile before authorized task
creation and pass only its non-null model and thinking values. Do not recommend
models, reasoning depths, or plan-based combinations. Do not choose compute from
perceived task difficulty or store environment-specific model names in project files.
