...
| The first software Ticket needs a test boundary, the Project Profile is missing or drifting, or the user asks how much testing a change needs | Apply registered `validation-design`; require the Product Owner to select current use, exposure, delivery, and system size before forming the proof | Current Project Lead |
