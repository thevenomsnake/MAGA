...
test("install command installs only the plugin", async () => {
  ...
  await runCli(["install"], {
    installPlugin: () => {
      installs += 1;
      return { name: "maga", version: "0.9.0" };
    },
    write: (value) => { output += value; },
  });

  assert.equal(installs, 1);
  assert.equal(output, "Installed MAGA 0.9.0.\n");
});
...
test("plugin installer configures its marketplace before installation", () => {
  ...
  const execute = (command, args) => {
    ...
    if (args.join(" ") === "plugin add maga@maga --json") {
      return JSON.stringify({ name: "maga", version: "0.9.0" });
    }
    return "{}";
  };
  ...
  assert.deepEqual(result, { name: "maga", version: "0.9.0" });
