...
- 向已创建任务发送项目入口与启动消息；
- Project Lead 再通过 Codex 原生项目工具创建同项目职责与使命任务，管理等待、续发、完成与归档，不要求用户手工调度会话。
...
已经在 V1.5-V3 落定的默认值不再列为开放问题：任务权限在项目状态中使用 `pending / approved / revoked`，授权范围是已描述的使命集；只有多使命队列、独立长期上下文或权限边界才建立岗位管理任务；Project Lead 和岗位管理任务置顶，完成的使命 worker 在持久状态收口后归档。
