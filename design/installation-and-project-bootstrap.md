...
> 状态：讨论草案。本文保存目标产品体验、当前事实和建议路线，不表示这些能力已经实现。
...
官方资料尚未建立以下保证：一个独立安装命令创建的 App Server thread，会自动出现在用户当前 Codex Desktop 项目的左侧栏中。

因此，“安装命令完成后左侧立即出现全部会话”应视为待验证的产品目标，而不是第一版承诺。当前最可靠的路径是：初始化命令准备项目，用户在 Codex 中打开该目录并启动一次 Project Lead，后续会话由这个活跃任务创建。
...
- 验证官方 Desktop 与 App Server 的稳定线程归属关系；
- 若可可靠接入，由初始化器创建并命名 Project Lead；
- 若不可可靠接入，保留一次打开目录或点击入口的产品步骤，不伪装成全自动。

### V3：独立客户端，仅在必要时

只有官方 Codex 项目与侧边栏无法承载目标体验时，才考虑使用 App Server 构建自己的启动器或客户端。第一版不承担这项复杂度。
