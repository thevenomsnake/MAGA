...
MAGA 会把模型选择留在普通产品对话之外，但不会把这项决定藏起来。你不需要懂代码，也不需要自己管理 7 个人；这些名称只是 MAGA 用来分工的内部标签。

### 用一个现实例子理解 7 个职责

假设一家社区美发店提出：“我想让顾客在网上预约，少打电话，但同一个时段不能被订两次。” MAGA 会像一支完整的产品团队那样推进：

- **Project Lead (`project-lead`) — 项目总管：**把这句话变成清楚的第一版目标，例如“顾客能选择空闲时段，店员能看到预约，而且系统会阻止重复预约”，再决定先做什么并协调后续工作。
- **研究 (`research`) — 调研员：**了解顾客和店员现在怎样预约，比较同类服务，并确认取消预约、保存顾客资料等现实规则。
- **原型 (`prototype`) — 彩排与样板：**先做一套可以点着试的预约流程，让店主在正式投入前亲手试用，判断步骤是否顺手。
- **交付 (`delivery`) — 施工团队：**把通过的样板真正做成可运行的产品，让顾客能够预约，也让店员能够管理时段。
- **诊断 (`diagnosis`) — 维修技师：**如果出现“顾客看到预约成功，却一直没收到确认”之类的问题，找出故障究竟发生在哪一环，而不是盲目重做。
- **审查 (`review`) — 验收员：**检查是否仍可能重复预约、有没有漏掉店主提出的要求，以及顾客资料是否得到妥善保护。
- **发布 (`release`) — 开业负责人：**确认原有资料已有备份、只有正确的人能进入管理后台、上线步骤已经准备好，再开放给真实顾客。

配置模型时，你只是在决定每一类工作可以使用多少判断与推理能力；实际选择职责和协调工作的事情由 MAGA 完成。
