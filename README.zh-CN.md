...
6. **设计：** 能够跨任务恢复的已确认产品与系统 design records。
7. **连续推进：** 有边界的只读 subagent、任务分派，以及带停止条件的 thread Goal。
...
- 对已批准 Ticket 内的短问题，MAGA 可以使用最多两个原生只读 subagent。它们不能写文件、提交、创建任务、发布内容或扩大范围；宿主不支持时，就回到当前任务或改用具名 worker。
- 如果项目已经确认 Autonomy Policy，MAGA 可以在已批准 Ticket 内按该项目确认的上限创建具名 worker，并传递有边界的 context packet；建议起始上限为 2。它不会自动创建新 Ticket 或扩大范围。
- thread Goal 只负责在当前已批准目标内继续工作，不会授权权限、发布或新范围。
...
保存后的修改只影响你用产品语言明确批准的新任务；如果项目已经确认 Autonomy Policy，MAGA 也可以在已批准 Ticket 内按该项目的 worker limit 创建具名 worker，并传递有边界的 context packet。已有任务保持原设置。Project Lead 只会在首次新建，或你明确要求用新配置创建 replacement 接手项目时采用保存的配置。面板中的 `model/list` 只是参考目录，不代表每个目标环境的最终能力。MAGA 会把你保存的模型和深度交给新任务的目标宿主验证；若目标宿主拒绝，才省略 overrides 重试一次，并明确告知使用宿主默认值。它不会擅自换成另一档模型，也不会因为任务“看起来很难”就升级。

## 里面有什么

当前版本是 **v0.16.0**，包含 19 个注册 Skills、有边界的主动任务协调、原生只读 subagent 协作与子任务恢复、项目内 design records、可选的 thread Goal continuation、一套只在需要时加载的内部方法库、职责级计算配置、由产品负责人确认的 Bar Tester 项目画像、面向本地文件文字的 Humanization 自动路由，以及基于明确提交的 Git 交付保护。
...
| Bar Tester | 先确认产品今天真正如何使用，再选择足够且最小的验证，包括“酒单都测完了，真实用户却点了炒饭”这类预期输入清单之外的边界 |
| 产品发现 | 澄清、研究、领域语言、概念和优先级 |
| 设计与交付 | 规划、原型、实现、验证和收尾 |
| 项目 memory | 持久产品状态、已确认 design records、Ticket 授权和恢复指针 |
...
默认在当前任务继续工作。项目确认 Autonomy Policy 后，MAGA 可以在该项目的 worker limit 内为已批准 Ticket 创建具名 worker，并传递持久上下文；没有该 policy 时，仍会先询问具体名称。它不会预建空的研究、原型、实现或 review 房间。

### 授权

自然语言授权适用于当前清楚描述的产品切片。Autonomy Policy 可以覆盖该 Ticket 内的 task 创建，但不会授权未来 Tickets、范围明显扩张后的结果或外部/不可逆动作。
