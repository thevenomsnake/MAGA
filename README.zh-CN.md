...
MAGA 面对的是一整个应用，但这个应用不必是一座庞大的平台。它会协调一个目标明确的小产品从意图走向可运行发布所需的完整工作。你不需要把下面 7 个名称当成必须学习的岗位，也不需要自己管理 7 支团队；它们只是 MAGA 用来分工的内部标签。

### 用一个小而完整的产品理解 7 个职责

假设你想从零上线一个给小型兴趣社区使用的动态应用：用户可以注册、填写昵称和简介、发布短动态、在首页看到社区的新内容并回复。它不是给现有产品增加一个按钮，也不是一开始就建造庞大的社交平台，而是一个目标明确、核心流程完整、可以交给真实用户使用的小产品。

- **Project Lead (`project-lead`) — 推进整个产品：**把你的产品方向整理成清楚的范围和验收标准，例如先完成“注册 → 填写资料 → 发布动态 → 在首页看到动态 → 回复”的核心闭环，再协调研究、原型、实现和验证。影响目标用户或体验方向的决定仍然由你做。
- **研究 (`research`) — 为产品决定寻找依据：**了解这个兴趣社区现在怎样交流、现有方式哪里不方便，以及成员对个人资料、短动态、首页内容和回复方式有什么真实预期，帮助你避免凭空设计。
- **原型 (`prototype`) — 在正式构建前看见并试用产品：**先做出可以操作的注册、资料、发布、首页和回复流程，让你亲手检查信息是否清楚、步骤是否顺手、整个核心循环是否成立。
...
- **审查 (`review`) — 独立检查产品是否完整可靠：**从注册开始走完发布、浏览和回复的完整流程，检查结果是否符合你的要求，并确认账户资料和社区内容具备必要的无障碍、隐私与安全边界。
- **发布 (`release`) — 把产品可靠地交给真实用户：**确认线上设置、资料备份、运行观察和出现问题时的撤回方式已经准备好。得到你的上线批准后，再正式开放产品，并验证新用户能够完成整个核心流程。
