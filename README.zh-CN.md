...
MAGA 会把模型选择留在普通产品对话之外，但不会把这项决定藏起来。你不需要理解工程团队的分工，也不需要亲自把任务派给 7 个人；这些名称只是 MAGA 用来区分不同工作性质的内部标签。

### 用一个互联网产品例子理解 7 个职责

假设你正在设计一款经典时间线社交产品，希望加入“私密收藏”。用户看到一篇长帖却暂时没空读；点赞会公开表达态度，截图又很难整理。因此，用户需要把帖子私下保存起来，并且刷新页面或换设备后仍然找得到，其他人也不能看到他的收藏。

- **Project Lead (`project-lead`) — 产品推进负责人：**把想法整理成可验收的第一版，例如“登录用户可以私密收藏和取消收藏；刷新或换设备后结果保持一致；作者和其他用户都看不到收藏行为”，再明确待决定的问题、安排先后顺序并协调后续工作。
- **研究 (`research`) — 收集决策依据：**了解用户为什么不愿用点赞或截图代替收藏，研究他们希望从哪里收藏、之后怎样找回来，并比较同类产品对私密性、内容失效和多设备同步的处理方式。
- **原型 (`prototype`) — 提前试用体验：**先做出可操作的收藏入口、保存反馈和“我的收藏”列表，让你判断按钮是否会被误解成点赞、私密提示是否可信、找回长帖是否顺手。
- **交付 (`delivery`) — 做成真实功能：**把确认过的体验变成可使用的产品，并把收藏可靠地记录到当前用户的账户中，而不只是让页面上的图标暂时改变。
- **诊断 (`diagnosis`) — 找出故障原因：**如果出现“刚收藏时看得到，刷新后却消失”“电脑上收藏，手机上没有”或“收藏状态显示错误”等问题，重现现象并定位是保存、读取、账户同步还是界面状态出了问题。
- **审查 (`review`) — 独立检查是否可靠：**对照目标检查收藏、取消收藏、重复操作、退出后重新登录和帖子被删除等情况，尤其确认作者、关注者和其他账户无法从时间线、通知或个人资料中发现收藏行为。
- **发布 (`release`) — 把关真实上线：**确认分批开放、效果观察和出现问题时的撤回方案已经准备好，再向合适范围的真实用户开放，并验证收藏不会丢失，也没有泄露私密行为。

配置模型时，你只是在决定每一类工作可以使用多少判断与推理能力；实际识别职责、安排任务和协调推进由 MAGA 完成。
