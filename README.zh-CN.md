...
MAGA 是一个可安装到 Codex Desktop 的产品构建工作流。你用产品语言描述用户、问题、体验、限制和取舍；一个持续工作的 Project Lead 会选择合适的方法，组织研究、原型、实现、验证和修复。
...
严格来说，MAGA 不是第二个应用，也不替换 Codex 的界面。Codex Desktop 仍然是唯一界面。所谓“套壳”，是安装后的一份工作契约：它规定产品意图如何变成被协调的工程执行。
