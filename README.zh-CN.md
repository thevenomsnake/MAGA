...
MAGA 面对的是一整个应用：它会协调从产品意图到可运行发布所需的完整工作。增加单个功能，只是这条旅程中可能出现的一小段。你不需要把下面 7 个名称当成必须学习的岗位，也不需要自己管理 7 支团队；它们只是 MAGA 用来分工的内部标签。

### 用一个完整产品理解 7 个职责

假设你想从零构建并上线一款经典的时间线社交应用。它面向希望围绕共同兴趣持续交流的人，让用户能够注册、建立个人资料、关注他人、发布动态、在首页看到所关注账号的内容并参与回复，同时具备必要的隐私与安全边界。

- **Project Lead (`project-lead`) — 推进整个产品：**把你的产品方向整理成阶段和验收标准，例如先建立“注册 → 完善资料 → 关注 → 发布 → 在首页看到动态 → 回复”的核心循环，再协调研究、设计、实现和验证。遇到会改变目标用户、产品体验或商业方向的取舍时，它会把决定交还给你。
- **研究 (`research`) — 为产品决定寻找依据：**弄清目标用户现在在哪里交流、现有方式为什么不够好、他们为什么愿意迁移到新产品，并比较同类产品在关注关系、时间线、回复、隐私和内容治理上的做法与代价。
- **原型 (`prototype`) — 在正式构建前看见和试用完整体验：**先做出可以操作的注册、个人资料、关注、发布、首页时间线、回复、举报和屏蔽流程，让你亲手检查信息结构与核心旅程，再决定是否进入实现。
- **交付 (`delivery`) — 把确认过的产品逐步做成真的：**把通过验收的产品切片变成可运行的应用，让账户、个人资料、关注关系、动态、时间线和回复能够真实保存并相互连接，而不只是停留在图片或演示中。
- **诊断 (`diagnosis`) — 找出实际故障发生在哪里：**如果用户明明关注了某个人，首页却看不到他的动态；刷新后刚发布的内容消失；或一个账户看到了不属于自己的资料，它会重现问题并定位真正原因，而不是盲目重做整个应用。
- **审查 (`review`) — 独立检查产品是否真的成立：**从用户目标和风险两方面检查完整体验，包括注册与登录、编辑个人资料、关注与取消关注、发布与删除、时间线是否正确、回复是否可控，以及举报、屏蔽、无障碍、隐私和账户隔离等基本边界有没有遗漏。
- **发布 (`release`) — 把产品可靠地交给真实用户：**确认线上配置、访问权限、数据迁移、资料备份、运行观察和出现问题时的撤回方案已经准备好。得到你的上线批准后，再正式开放产品，并验证新用户能够从头走通核心旅程。
