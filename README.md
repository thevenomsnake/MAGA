...
<p align="center">
  <img src="./assets/kann-workflows-routing-hero.png" alt="Kann Workflows 自动路由：一个产品主理协调四类按需工作区" width="100%">
</p>
...
Kann 不是靠一组固定关键词把用户塞进预设流水线。它使用两层路由：
...
2. **Kann 项目层**：Project Lead 被加载后，结合用户本轮意图与仓库中的
...
- **Matt 的 9 个隐式入口**：`prototype`、`diagnosing-bugs`、`research`、`tdd`、
  ...
  `resolving-merge-conflicts`、`grilling`。Kann 项目额外规定：除非用户明确要求，
  普通交付不默认采用 TDD。
- **Matt 的 13 个显式入口**：`ask-matt`、`grill-with-docs`、`triage`、
  ...
  高级用户可以显式选择。Project Lead 也可以在 Kann 自己的授权与项目边界内，
  把相应说明作为内部方法采用——这是 Kann 的编排，不是 Matt 上游自动调用。
...
分布在 [能力与工作区路由](plugins/kann-workflows/skills/project-lead/references/capability-routing.md)、
[项目记忆](plugins/kann-workflows/skills/project-lead/references/project-memory.md)、
[原生 Codex 闭环](plugins/kann-workflows/skills/project-lead/references/native-codex-loop.md)
和 [Ticket 编排](plugins/kann-workflows/skills/orchestrate-tickets/SKILL.md)。每个上游
入口的用途与自然语言示例见
[Matt Pocock Skills 与 Ponytail 使用手册](playbooks/matt-skills-and-ponytail-guide.md)。

```mermaid
flowchart TD
    accTitle: Kann Workflows 自动路由流程
...
| 识别到的信号 | Kann 选择的方法 | 默认工作区 | 何时不会进入该路由 |
| --- | --- | --- | --- |
...
| 结果需要跨会话、跨职责或长期保存 | 建立面向用户结果的 Kann Tickets | 先写入仓库状态，不一定创建新任务 | 几分钟内可在当前聚焦任务完成时不制造 Ticket 仪式 |
...
不是合格的 worker 标题。每次创建前，Kann 都会按项目与确定性标题查找已有任务，
...
- 如果当前 Codex 环境没有任务协调工具，Kann 会留在当前任务完成可完成的部分，
