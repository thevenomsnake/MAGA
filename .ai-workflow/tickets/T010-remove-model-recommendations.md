...
- [ ] 删除职责推荐值、套餐/额度/质量预设、推荐按钮与标签，以及接口返回的预设字段；同时删除失去调用方的实现。
- [ ] 未配置与部分配置中的未设置职责显示继承宿主默认值，创建任务时省略相应 overrides；保存操作不把未设置职责补成具体模型。
- [ ] 保留旧的完整显式配置与用户主动使用的中性配置入口；支持未配置职责的继承状态而不重置旧配置，不自动改写外部配置文件。
- [ ] 中性模型列表只显示宿主实际支持的选项，不标首选、不做推荐排序；拒绝显式配置时保留原有有界回退，只说明事实。
- [ ] 同步清理 README、全部语言指南、网站源文件、MCP 说明、Skill、模板及公开研究/设计资料中的选型建议，不重写 Git 历史。
- [ ] 保留协议必需的模型标识与测试样例，取消任何新增推荐预设计划。
