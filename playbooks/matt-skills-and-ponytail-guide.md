...
- 19 个注册 Skills，供 Codex 宿主识别和按需加载；其中 MAGA 原生
  `bar-tester` 会先判断并推荐项目画像，再请用户确认当前用途、暴露面、交付形式和规模；
- 13 个 Matt 原手动流程，作为 Project Lead 的内部方法；
- 3 个吸收能力：Ponytail 的 help、gain 信息卡，以及 `wizard` 的人工门禁内核；
- Humanization 实际文本产出自动路由，以及 Ponytail 会话启动、恢复、清空、压缩、模式切换
  和子任务继承 hooks。

维护 catalog 共记录 35 个能力：19 个 registered、13 个 internal method 和 3 个
absorbed。
...
这 11 个能力保留上游技术 ID，因为它们拥有不同的输入、证据和完成边界。前 10 个
延续上游隐式调用；`wait-what` 保留上游重讲内核，但由 MAGA 从手动调用适配为隐式
调用：
...
| `wait-what` | 用户以任何语言表示上一段解释没有讲明白，需要补足前提并换一种方式重讲 |
...
1. 插件页面列出 19 个 Skills，展示名能够清楚识别 MAGA、Humanization 或 Ponytail；
2. catalog 精确包含 35 个能力：19 registered、13 internal method、3 absorbed；
3. `humanization`、`research`、`prototype`、`diagnosing-bugs`、`writing-for-agents`、`wait-what` 各用一个自然语言
   正例触发；
...
`0.14.0` 不保留被收进内部方法库或吸收层的旧技术入口别名；`wait-what` 是明确重新
...
4. 验证 19 个注册入口、13 个内部方法、3 个吸收映射、35 项 catalog、触发正反例与
