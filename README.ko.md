...
MAGA는 일반적인 제품 대화에서는 모델 선택을 신경 쓰지 않게 하면서도 그 결정을 숨기지 않습니다. 이 일곱 가지는 사용자가 배워야 할 개발 직무도, 일곱 명을 따로 고용하라는 뜻도 아닙니다. MAGA가 일을 알맞게 나누기 위해 쓰는 내부 라벨입니다.

### 현실적인 예: 동네 미용실이 온라인 예약을 시작하는 경우

동네 미용실이 “고객이 전화하지 않고 예약할 수 있게 해 주세요”라고 요청한 상황을 생각해 보세요.

- **Project Lead (`project-lead`) — 전체를 이끄는 총괄:** 목표를 듣고 첫 번째 버전에 꼭 필요한 범위를 정한 뒤, 다음에 무엇을 할지 결정합니다.
- **조사 (`research`) — 현장을 알아보는 시장 조사원:** 고객이 지금 어떻게 예약하는지, 다른 매장은 어떻게 하는지 살펴보고 마감된 시간과 취소를 어떻게 처리해야 하는지 확인합니다.
- **프로토타입 (`prototype`) — 본 공사 전의 견본:** 실제로 만들기 전에 눌러 볼 수 있는 예약 흐름을 준비해 점주가 불편한 부분을 미리 발견하도록 합니다.
- **전달 (`delivery`) — 실제로 완성하는 제작팀:** 승인된 견본을 시간 선택, 예약, 확인 알림이 실제로 작동하는 서비스로 만듭니다.
- **진단 (`diagnosis`) — 고장을 찾는 수리 기사:** 예약은 기록됐는데 확인 알림이 오지 않는다면, 과정이 어디서 멈췄는지 찾아 원인을 고칩니다.
- **리뷰 (`review`) — 결과를 확인하는 검사 담당:** 중복 예약이 생기지 않는지, 개인정보가 보호되는지, 점주가 요청한 내용이 빠짐없이 반영됐는지 확인합니다.
- **릴리스 (`release`) — 문을 열기 전의 최종 점검 담당:** 권한, 백업, 문제 발생 시 되돌리는 방법을 확인한 뒤 고객이 서비스를 사용할 수 있게 합니다.

아래 표는 사용자가 일곱 가지 업무를 직접 관리하라는 뜻이 아닙니다. MAGA가 각 상황에서 어떤 AI 설정에 일을 맡길지를 보여 줍니다.
