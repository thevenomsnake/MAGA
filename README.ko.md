...
MAGA는 기능 하나만 추가하는 도구가 아닙니다. 그렇다고 거대한 소셜 플랫폼을 상상할 필요도 없습니다. 작지만 처음부터 끝까지 사용할 수 있는 제품을 처음부터 만들 수 있습니다.

### 작지만 완결된 사례: 관심사 소식 공유 앱을 처음부터 출시하기

관심사가 같은 사람들을 위한 작은 앱을 생각해 보세요. 사용자는 가입하고 간단한 프로필을 만들며, 짧은 소식을 게시하고, 홈에서 커뮤니티의 새 글을 읽고, 답글을 달 수 있습니다. 이 흐름만으로도 실제로 공개해 사용할 수 있는 완전한 첫 버전이 됩니다.

이 일곱 가지는 제품 디자이너가 모두 익혀야 할 기술 직무도, 일곱 명을 직접 관리해야 한다는 뜻도 아닙니다. 사용자가 제품을 정의하고 중요한 결정을 내리며 결과를 확인하는 동안, MAGA가 보이지 않는 곳에서 일을 정리하기 위해 사용하는 내부 라벨입니다.

- **Project Lead (`project-lead`) — 제품 전체를 하나로 묶습니다:** 아이디어를 명확한 결과로 정리하고, 누구를 위해 어떤 가치를 만들지와 첫 번째 완전한 버전에 무엇을 포함할지 정합니다. 다음에 조사·시험·구현·결정할 일도 조율합니다.
- **조사 (`research`) — 판단에 필요한 근거를 모읍니다:** 커뮤니티가 무엇을 공유하고 싶은지, 새 게시물을 어떻게 발견하고 싶은지, 어떤 답글이 유용한지 알아보며 구축 전의 가정을 줄입니다.
- **프로토타입 (`prototype`) — 경험을 검토할 수 있게 만듭니다:** 가입과 프로필부터 게시, 홈의 새 글, 답글까지 이어지는 흐름을 직접 시험할 수 있는 형태로 만들어 실제 구축 전에 제품 방향을 점검합니다.
- **전달 (`delivery`) — 합의한 방향을 실제 제품으로 완성합니다:** 계정, 프로필, 게시물, 홈의 새 글, 답글이 처음부터 끝까지 작동하고 정보가 올바르게 유지되는 최소한의 완전한 제품을 만듭니다.
- **진단 (`diagnosis`) — 문제와 원인을 찾아냅니다:** 게시물이 사라지거나, 홈이 갱신되지 않거나, 답글이 다른 게시물에 표시될 경우 어느 지점에서 실패했는지 찾아 원인을 고칩니다.
- **리뷰 (`review`) — 제품이 약속한 결과를 검증합니다:** 사용자가 주요 흐름을 끝까지 완료할 수 있는지, 중요한 상태가 빠지지 않았는지 근거로 확인하고 접근성, 개인정보 보호, 안전의 기본 경계도 점검합니다.
- **릴리스 (`release`) — 실제 사용자에게 제품을 전달합니다:** 완료 범위, 알려진 위험, 문제를 발견하고 되돌리는 방법을 확인합니다. 사용자의 승인 후 커뮤니티가 실제로 사용할 수 있는 환경에 앱을 공개합니다.
