...
MAGA는 일반적인 제품 대화에서는 모델 선택을 신경 쓰지 않게 하면서도 그 결정을 숨기지 않습니다. 이 일곱 가지는 제품 디자이너가 모두 익혀야 할 기술 직무도, 일곱 가지 업무를 직접 관리해야 한다는 뜻도 아닙니다. 제품 판단을 MAGA가 실행 가능한 일로 나누기 위해 사용하는 내부 라벨입니다.

### 제품 사례: 타임라인형 소셜 앱에 ‘비공개 저장’ 추가하기

전형적인 타임라인형 소셜 앱에서 사용자가 긴 게시물을 발견했지만 지금은 읽을 시간이 없는 상황을 생각해 보세요. ‘좋아요’를 누르면 반응이 공개되고, 스크린샷은 나중에 정리하기 어렵습니다. 그래서 제품에 **‘비공개 저장’** 기능을 추가합니다. 저장한 게시물은 새로고침하거나 다른 기기로 옮겨도 사라지지 않아야 하며, 본인 외에는 누구에게도 보여서는 안 됩니다.

- **Project Lead (`project-lead`) — 결과 전체를 조율합니다:** “쉽게 저장하고, 나중에 다시 찾으며, 기기를 바꿔도 남아 있고, 다른 사람에게는 보이지 않아야 한다”는 검증 가능한 약속으로 정리합니다. 범위를 지키고 다음에 해결할 질문을 결정합니다.
- **조사 (`research`) — 필요와 기대를 확인합니다:** 사용자가 ‘좋아요’나 스크린샷 대신 저장을 선택하는 순간, 저장한 게시물을 다시 찾는 방식, 사용자가 기대하는 ‘비공개’의 의미를 알아봅니다.
- **프로토타입 (`prototype`) — 경험을 검토할 수 있는 형태로 만듭니다:** 저장 동작, 완료 표시, 비공개 저장 목록, 저장 해제, 빈 화면까지 직접 눌러 볼 수 있는 흐름으로 만들어 실제 구현 전에 경험을 점검합니다.
- **전달 (`delivery`) — 합의한 범위를 실제 기능으로 완성합니다:** 승인된 흐름을 작동하는 기능으로 만들고, 저장한 게시물을 올바른 계정에 연결해 새로고침하거나 다른 기기에서 열어도 남고 다른 사람에게는 보이지 않도록 합니다.
- **진단 (`diagnosis`) — 문제가 생긴 원인을 찾아냅니다:** 저장한 게시물이 새로고침 후 사라지거나, 다른 기기에 나타나지 않거나, 실수로 다른 사람에게 노출될 경우 약속이 깨진 지점을 찾아 원인을 고칩니다.
- **리뷰 (`review`) — 제품이 약속한 결과를 검증합니다:** 저장, 다시 찾기, 해제, 중복 방지, 기기 간 반영, 비공개성이 합의대로 작동하는지 근거로 확인하고 접근성도 점검합니다.
- **릴리스 (`release`) — 사용자에게 공개할 준비가 됐는지 판단합니다:** 완료 범위, 개인정보 보호 위험, 공개 후 확인 방법, 문제 발생 시 되돌리는 방법을 확인한 뒤 기능을 활성화합니다.

아래 표는 각 책임을 어떤 AI 설정에 맡길지 보여 줍니다. MAGA가 작업을 조율하며, 제품 판단과 결과 승인 권한은 계속 사용자에게 있습니다.
