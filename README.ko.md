...
MAGA가 다루는 단위는 앱 전체입니다. 첫 아이디어에서 시작해 실제 사용자가 접속할 수 있는 제품을 공개할 때까지 전 과정을 조율합니다. 기능 하나를 추가하는 일은 그 여정의 한 부분일 뿐입니다.

이 일곱 가지는 제품 디자이너가 모두 익혀야 할 기술 직무도, 일곱 명을 직접 관리해야 한다는 뜻도 아닙니다. 사용자가 제품을 정의하고 중요한 결정을 내리며 결과를 확인하는 동안, MAGA가 보이지 않는 곳에서 일을 나누기 위한 내부 구조입니다.

### 완전한 사례: 타임라인형 소셜 앱을 처음부터 만들어 출시하기

특정 커뮤니티를 위한 전형적인 타임라인형 소셜 앱을 처음부터 만든다고 생각해 보세요. 사용자는 가입하고 프로필을 작성하며, 다른 계정을 팔로우하고, 게시물을 올리고, 홈 타임라인을 읽고, 답글을 달 수 있어야 합니다. 차단이나 신고 같은 기본적인 안전 경계도 필요합니다.

- **Project Lead (`project-lead`) — 제품 전체를 하나로 묶습니다:** 구상을 “누구에게 어떤 가치를 제공할 것인가”라는 구체적인 결과로 정리하고, 처음 출시할 완전한 범위를 정한 뒤 다음에 조사·시험·구현·결정할 일을 조율합니다.
- **조사 (`research`) — 판단에 필요한 근거를 모읍니다:** 목표 사용자와 현재 사용하는 대안, 팔로우·게시·대화에 대한 기대를 알아보고, 개인정보 보호, 운영 정책, 플랫폼 조건도 확인합니다.
- **프로토타입 (`prototype`) — 전체 경험을 검토할 수 있게 만듭니다:** 가입, 프로필, 팔로우, 게시, 타임라인, 답글, 차단과 신고까지 이어지는 흐름을 직접 시험할 수 있는 형태로 만들어 실제 구축 전에 제품 방향을 점검합니다.
- **전달 (`delivery`) — 합의한 방향을 실제 제품으로 완성합니다:** 계정, 프로필, 관계, 게시물, 답글, 타임라인이 처음부터 끝까지 작동하고 정보가 올바르게 유지되는 최소한의 완전한 제품을 만듭니다.
- **진단 (`diagnosis`) — 문제와 원인을 찾아냅니다:** 게시물이 사라지거나, 타임라인에 잘못된 내용이 나오거나, 팔로우가 반영되지 않거나, 안전 경계가 깨질 경우 어느 지점에서 제품이 실패했는지 찾아 원인을 고칩니다.
- **리뷰 (`review`) — 결과를 의심하고 검증합니다:** 주요 흐름이 제품 의도를 충족하는지, 중요한 상태가 빠지지 않았는지 근거로 확인하고 접근성, 개인정보 보호, 기본적인 안전성도 점검합니다.
- **릴리스 (`release`) — 실제 사용자에게 제품을 전달합니다:** 완료 범위, 알려진 위험, 공개 설정, 데이터 이전, 운영 중 확인 방법, 문제 발생 시 복구 방법을 확인합니다. 사용자의 승인 후 실제 가입하고 이용할 수 있는 환경에 앱을 공개합니다.

이 책임들은 MAGA가 보이지 않는 곳에서 조율합니다. 사용자는 일곱 직무를 관리할 필요 없이 사용자, 경험, 우선순위, 경계, 결과라는 제품 언어로 계속 판단하면 됩니다.
