...
| Bar Tester | 지금 제품이 실제로 어떻게 쓰이는지 확인한 뒤 필요한 만큼만 검증한다. ‘술 주문은 모두 확인했지만 볶음밥 주문은 놓친’ 것처럼 예상 입력 목록 밖의 경계도 포함한다 |
| 제품 발견 | 확인, 조사, 도메인 언어, 개념, 우선순위 |
| 설계와 전달 | 계획, 프로토타이핑, 구현, 검증, 완료 |
| 프로젝트 memory | 지속 제품 상태, 승인된 design records, Ticket 권한, 복구 포인터 |
...
`wait-what`은 자동 호출을 유지합니다. 사용자가 어떤 언어로든 이전 설명을 이해하지 못했다고 표현하면 MAGA는 현재 대화에서 빠진 전제를 보충하고 더 명확한 표현과 프로젝트 용어로 다시 설명합니다. `$wait-what`을 명시적으로 호출할 수도 있습니다. Codex 호스트가 지원하면 Skill 표시가 나타날 수 있지만 복구 동작은 그 UI 표시에 의존하지 않습니다.
...
기본적으로 현재 작업에서 계속 진행합니다. 확인된 Autonomy Policy가 있으면 승인된 Ticket 안에서 프로젝트의 worker limit까지 이름 있는 worker를 만들 수 있습니다. Policy가 없으면 구체적인 작업 이름을 먼저 확인합니다. 비어 있는 조사, 프로토타입, 구현, 리뷰 공간을 미리 만들지 않습니다.

### 권한

자연어 승인은 명확히 설명된 현재 제품 단위에 적용됩니다. Autonomy Policy는 해당 Ticket 안의 task 생성만 보완하며, 이후 Tickets나 크게 확장된 결과, 외부 또는 되돌릴 수 없는 작업까지 승인하지 않습니다.
